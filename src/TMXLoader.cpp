...
SDL_Surface* TMXLoader::load_SDL_image( std::string filename )
{
    ...
    if( loadedImage != NULL )
    {
        ...
        optimizedImage = SDL_ConvertSurfaceFormat( loadedImage, SDL_GetWindowPixelFormat(GWindow), 0);
        ...
        {
            //Color key surface
            SDL_SetColorKey( optimizedImage, true, SDL_MapRGB( optimizedImage->format, 0, 0xFF, 0xFF ) );
        }
    }
